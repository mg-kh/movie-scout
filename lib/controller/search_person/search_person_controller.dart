...
class SearchPersonController extends GetxController {
  ...
  Future getSearchPersonData({pageNumber: 1, required query}) async {
    isErrorOccur(true);
    queryData(query);
    ...
    if (resultPersonData.length > 0 && pageNumber == 1) {
      resultPersonData.value = [];
    }


    try {
      var tvShow = await SearchPersonApiService().remoteSearchPersonData(pageNumber: pageNumber, query: query);
      print(tvShow);

      resultPersonData.value = [...resultPersonData, ...tvShow.results];
