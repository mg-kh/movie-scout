...
class SearchMovieController extends GetxController {
  ...
  Future getSearchMovieData({pageNumber: 1, required query}) async {
    isErrorOccur(true);
    queryData(query);
    ...
    if (resultMovieData.length > 0 && pageNumber == 1) {
      resultMovieData.value = [];
    }


    try {
      var tvShow = await SearchMovieApiService().remoteSearchMovieData(pageNumber: pageNumber, query: query);
      print(tvShow);

      resultMovieData.value = [...resultMovieData, ...tvShow.results];
